...
import sys
import numpy as np
from skeleton_utils.tree_utils import TreeNode
...
def primMST_symmetry(graph, init_id, joints):
    ...
    """
    joint_mapping = {}
    left_joint_ids = np.argwhere(joints[:, 2] < -2e-2).squeeze(1).tolist()
    middle_joint_ids = np.argwhere(np.abs(joints[:, 0]) <= 2e-2).squeeze(1).tolist()
    right_joint_ids = np.argwhere(joints[:, 2] > 2e-2).squeeze(1).tolist()
    print('left_joint_ids = ', left_joint_ids)
    print('right_joint_ids = ', right_joint_ids)
    for i in range(min(len(left_joint_ids),len(right_joint_ids))):
...
def from_line_read_trans(line):
    ...
    trans_str = line[start+1:-2]
    trans_list = trans_str.split()
    trans = np.zeros(3)
    for i in range(3):
        trans[i] = float(trans_list[i])
    return trans 
    
            
def read_data_list(listf):
    data = None 
    with open(listf, 'r') as f:
        data = f.readlines()
    if data is None:
        print(listf, 'not exists!!')
        return
    ...
    for line in data:
        # print(line.split(','))
        data = line.split(' ')
        names.append(data[0] + '_' + str(data[1]).zfill(4))
        # print(data)
        scales.append(float(data[2]))
        trans_cat = ""
        for k in range(3,len(data)):
            trans_cat = trans_cat + data[k]
        
        tran = from_line_read_trans(line)
        trans.append(tran)
    return names, scales, trans


def write_to_obj(joints, parents, out_path):
    fid = open(out_path, 'w')
    for i in range(joints.shape[0]):
        print('v %f %f %f'%(joints[i,0], joints[i,1], joints[i,2]), file=fid)
    
    for i in range(len(parents)):
        if parents[i] >=0:
            print('l %d %d'%(i+1, parents[i]+1), file=fid)
    fid.close()


def gene_single_tree(joint_path):
    out_path = joint_path[:-4] + '_tree.npz'
    joints = om.read_trimesh(joint_path).points()
    distances = np.linalg.norm(joints[:, np.newaxis] - joints, axis=-1)

    probs = distances**2 # np.exp(-distances**2/0.01)

    # parents, key, initid = primMST_symmetry(probs, 2, joints)
    parents, key = primMST(probs, 2)
    parents_pos= []
    children_pos = []
    for i in range(len(parents)):
        if parents[i]>=0:
            parents_pos.append(joints[parents[i],:])
            children_pos.append(joints[i,:])
    np.savez(out_path, nodes=joints, parents=parents_pos, children=children_pos, parent_indices=parents)
    
    write_to_obj(joints, parents, out_path[:-4] + '_vis.obj')

def gene_tree(joints, probs):

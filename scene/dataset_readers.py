...
class CameraInfo(NamedTuple):
    ...
    height: int
    fid: float
    thinned: Optional[np.array] = None
    semantic_seg: Optional[np.array] = None  
    depth: Optional[np.array] = None
    K: Optional[np.array] = None
    reference_points: Optional[np.array] = None 
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, msk_folder=None, thinned_folder=None):
    cam_infos = []
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if thinned_folder is not None:
            thinned_path = os.path.join(thinned_folder, 'mask_' + image_name + '_thinned.png')
            thinned = np.array(Image.open(thinned_path).convert('L'))
            thinned_elements = np.concatenate(np.where(thinned > 0)).reshape(2,-1).transpose()
        else:
            thinned_elements = None 

        fid = int(image_name) / (num_frames - 1)
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height, fid=fid, thinned=thinned_elements)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", no_bg=False):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            thinned_path = os.path.join(path, 'train_thinned', image_name + '_thinned.png')
            
            seg_path = os.path.join(path, "semantic_seg", image_name + "_seg.npy") 
            if os.path.exists(seg_path):
                semantic_seg = np.load(seg_path)[0] #.astype(int)
            else:
                semantic_seg = None 
            
            
            if not os.path.exists(thinned_path):
                print('thinned path:', thinned_path, 'is not exists')
                thinned_elements = None 
            else:
                thinned = np.array(Image.open(thinned_path).convert('L'))
                thinned_elements = np.concatenate(np.where(thinned > 0)).reshape(2,-1).transpose()
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1], fid=frame_time, thinned=thinned_elements, semantic_seg=semantic_seg))
...
def readNerfiesCameras(path, inter_valid=False):
    ...
    if os.path.exists(f'{path}/masked_rgb'):
        print('Read RGBA images!')
        thinned_path = [f'{path}/train_thinned/{int(1 / ratio)}x/mask_{i}_thinned.png' for i in all_img] 
        all_img = [f'{path}/masked_rgb/{int(1 / ratio)}x/{i}.png' for i in all_img]
        msk_path = None
        # thinned_path = None
    ...
    if inter_valid:
        ...
        for idx in range(len(train_img)):
            ...
            if thinned_path is not None:
                thinned = np.array(Image.open(thinned_path[idx]).convert('L'))
                thinned_elements = np.concatenate(np.where(thinned > 0)).reshape(2,-1).transpose()
            else:
                thinned_elements = None
            ...
            cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1], fid=fid, thinned=thinned_elements)
            cam_infos.append(cam_info)
        ...
        for idx in range(cps_valid.shape[0]):
            ...
            cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1], fid=fids_valid[idx], thinned=thinned_elements)
            cam_infos.append(cam_info)
    else:
        cam_infos = []
...
def readNerfiesColmapCameras(path):
    ...
    if os.path.exists(f'{path}/masked_rgb'):
        img_path = f'{path}/masked_rgb/{int(1 / ratio)}x/'
        msk_path = None
        # thinned_path = None 
        thinned_path = f'{path}/train_thinned/{int(1 / ratio)}x/'
    else:
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder = img_path, msk_folder=msk_path, thinned_folder=thinned_path)
...
def apply_global_tfm_to_camera(E, Rh, Th):
    ...
    global_rot = cv.Rodrigues(Rh)[0].T
    global_trans = Th
...
def readZJUCameras(path, smpl_path, white_background=False, train_num=-1, with_smpl_pose=True, is_train=True):
    import pickle     
    ...
    for fname in cameras: 
        ...
        intrin = cameras[fname]["intrinsics"]
        extrin = cameras[fname]["extrinsics"]
        D = cameras[fname]['distortions']
        image = cv.undistort(image, intrin, D)
        mask = cv.undistort(mask, intrin, D)
        ...
        thinned_path = os.path.join(path, "train_thinned", fname + "_thinned.png") 
        if os.path.exists(thinned_path):
            thinned = np.array(Image.open(thinned_path).convert('L'))
            thinned_elements = np.concatenate(np.where(thinned > 0)).reshape(2,-1).transpose()
        else:
            thinned_elements = None
        ...
        reference_points_path = os.path.join(smpl_path, "SMPL_prior", fname + ".npy") 
        if os.path.exists(reference_points_path):
            reference_points = np.load(reference_points_path)
        else:
        ...
        if with_smpl_pose:
            dst_skel_info = query_dst_skeleton(mesh_infos[fname])
            extrin = apply_global_tfm_to_camera(
                    E=extrin, 
                    Rh=dst_skel_info['Rh'],
                    Th=dst_skel_info['Th'])
        ...
        cam_info = CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1],
                              fid=fid, thinned=thinned_elements, semantic_seg=semantic_seg, K=intrin.copy(), reference_points=reference_points)
...
def readZJUMovingMonocularCams(path, white_background):
    ...
    for idx in TEST_CAMERA_ID:
        test_path = os.path.join(path, 'test', 'view_'+str(idx).zfill(2))
        cam_infos, _, _, _ = readZJUCameras(test_path, path, white_background, train_num=train_num, with_smpl_pose=True, is_train=False)
        test_cam_infos += cam_infos 
    return train_cam_infos, test_cam_infos 

def readZJUInfo(path, white_background=False):
    print("Reading ZJU data Info")
    train_cam_infos, test_cam_infos = readZJUMovingMonocularCams(path, white_background)
    
    print("len_train_cam_infos = ", len(train_cam_infos), 'len_test_cam_infos = ', len(test_cam_infos))

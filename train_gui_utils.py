...
class DeformKeypoints:
    def __init__(self) -> None:
        ...
        self.keypoints_uv_list = [] # keypoints uv index
        self.idx2group = {}
        
        # parents 
        self.keypoints3d_parent_list = []  # list of keypoints' parents group
        self.keypoints_parent_idx_list = [] # keypoints' parents index
    ...
    def add_kpts(self, keypoints_coord, keypoints_idx, parents, cur_uv):
        ...
        for idx in range(len(keypoints_idx)):
            if not self.contain_kpt(keypoints_idx[idx].item()):
                ...
                self.keypoints_parent_idx_list.append(parents[keypoints_idx[idx]].item())
                self.keypoints_uv_list.append(cur_uv)
    ...
    def update_delta(self, nodes):
        # delta: [3,], np.array
        for idx in self.selective_keypoints_idx_list:
            # self.keypoints3d_delta_list[idx] += delta
            self.keypoints3d_delta_list[idx] = nodes[self.keypoints_idx_list[idx]] - self.keypoints3d_list[idx]

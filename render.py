...
def project_nodes_to_2d(viewpoint_cam, nodes, render_img,out_path,color='black'):
    ...
    if render_img is not None:
        ...
        mat = 255*torch.ones(3, height, width)
    
    for i in range(proj_x.shape[0]):
        if proj_y[i]>=height or proj_x[i]>=width:
            continue 
        
        if proj_y[i]<0 or proj_x[i]<0:
            continue 
        
        if color=='black':
            for k in range(3):
                mat[k, proj_y[i]-1:proj_y[i]+1, proj_x[i]-1:proj_x[i]+1] = 0 #[255,255,255] 
        elif color=='red':
            mat[0, proj_y[i]-1:proj_y[i]+1, proj_x[i]-1:proj_x[i]+1] = 255
            mat[1, proj_y[i]-1:proj_y[i]+1, proj_x[i]-1:proj_x[i]+1] = 0
            mat[2, proj_y[i]-1:proj_y[i]+1, proj_x[i]-1:proj_x[i]+1] = 0
        else:
            mat[0, proj_y[i]-1:proj_y[i]+1, proj_x[i]-1:proj_x[i]+1] = 255
            mat[1, proj_y[i]-1:proj_y[i]+1, proj_x[i]-1:proj_x[i]+1] = 255
            mat[2, proj_y[i]-1:proj_y[i]+1, proj_x[i]-1:proj_x[i]+1] = 255
            
    torchvision.utils.save_image(mat, out_path)
    return mat 
...
def render_set(model_path, load2gpt_on_the_fly, name, iteration, views, gaussians, pipeline, background, deform):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        write_to_obj(d_values['d_nodes'], os.path.join(pc_path, 'nodes_{0:05d}'.format(idx) + '.obj'))
        project_nodes_to_2d(view, d_values['d_nodes'], gt, os.path.join(pc_path, 'nodes_{0:05d}'.format(idx) + '_proj.png'))
        
    
    write_to_obj(deform.deform.nodes[:,:3], os.path.join(pc_path, 'nodes_template.obj'))
    write_to_obj(gaussians.get_xyz, os.path.join(pc_path, 'pc_template.obj'))

...
def project_nodes_to_2d_elements(viewpoint_cam, nodes):
    ...
    elements = torch.zeros(nodes.shape[0], 2).to(trans_nodes.device)
    proj_x = (fx * trans_nodes[:,0]/trans_nodes[:,2] + cx) #.int()
    proj_y = (fy * trans_nodes[:,1]/trans_nodes[:,2] + cy) #.int()
    elements[:,0] = proj_y 
    elements[:,1] = proj_x

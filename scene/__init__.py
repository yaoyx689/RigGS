...
class Scene:
    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], use_pretrain_model=False):
        """b
        ...
        model_path = self.pretrain_model_path if use_pretrain_model else self.model_path 
        print('model_path = ', model_path)
        if load_iteration:
            if load_iteration == -1:
                self.loaded_iter = searchForMaxIteration(os.path.join(model_path, "point_cloud"))
            else:
        ...
        if args.data_type=="ZJU":
            print("Assuming ZJU data set!")
            scene_info = sceneLoadTypeCallbacks["ZJU"](args.source_path, args.white_background)

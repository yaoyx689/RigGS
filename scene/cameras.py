...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid, trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda", fid=None, depth=None, thinned=None, semantic_seg=None, flow_dirs=[], K=None, reference_points=None):
        ...
        self.reference_points = torch.tensor(reference_points).to(self.data_device) if reference_points is not None else None 

        if gt_alpha_mask is not None:
            self.gt_alpha_mask = self.gt_alpha_mask.to(self.data_device)

        self.thinned = torch.Tensor(thinned).to(self.data_device) if thinned is not None else None 
        
        self.semantic_seg = torch.from_numpy(semantic_seg).to(self.data_device).int() if semantic_seg is not None else None         
        ...
        if self.K is not None:
            self.projection_matrix = getProjectionMatrix_from_K(znear=self.znear, zfar=self.zfar,  K=K, W=self.image_width, H=self.image_height).transpose(0, 1).to(self.data_device)
        else:

...
class ControlNodeWarp(nn.Module):
    ...
    def forward(self, x, t, feature, motion_mask, iteration=0, is_training=True, node_trans_bias=None, node_scaling_bias=None, animation_d_values=None, **kwargs):
        ...
        deformed_nodes = self.nodes[...,:3] + node_trans
        ##############################################################################
        if not self.d_rot_as_res:
            ...
            return_dict = {'d_xyz': translate, 'd_rotation': rotation, 'd_scaling': scale, 'd_nodes': deformed_nodes}
        else:
        ...
        # print('self.d_rot_as_res = ', self.d_rot_as_res, 'self.local_frame = ',self.local_frame, 'node_trans_bias = ',node_trans_bias, 'self.pred_opacity = ', self.pred_opacity, 'self.pred_color=',self.pred_color, 'reg_loss = ', self.reg_loss, 'lambda_arap = ', lambda_arap)
        # elf.d_rot_as_res =  True self.local_frame =  True node_trans_bias =  None self.pred_opacity =  False self.pred_color= False reg_loss =  tensor(3.3620e-06, device='cuda:0', grad_fn=<AddBackward0>) lambda_arap =  9.999999999999991e-05 [28/08 21:23:13]
        return return_dict

...
class GaussianModel:
    def __init__(self, sh_degree: int, fea_dim=0, with_motion_mask=True, use_isotropic_gs=False, **kwargs):
        ...
        self.use_isotropic_gs = use_isotropic_gs
        # print('use_isotropic_gs = ', use_isotropic_gs)
    ...
    def training_setup(self, training_args):
        ...
        self.skeleton_gs_position_lr = training_args.skeleton_gs_position_lr
    ...
    def update_learning_rate_for_skeleton_step(self, iteration):
        ''' Learning rate scheduling per step '''
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                param_group['lr'] = self.skeleton_gs_position_lr
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            stored_state = self.optimizer.state.get(group['params'][0], None)
            if stored_state is not None:
                # print('stored_state["exp_avg"].shape = ', stored_state["exp_avg"].shape, 'extension_tensor.shape = ', extension_tensor.shape)
    ...
    def sampling_and_prune(self, num_sample=5000):
        indices = farthest_point_sample(self.get_xyz[None], num_sample)[0]
        prune_mask = torch.ones(self.get_xyz.shape[0]) #.to(self.get_xyz.device)
        prune_mask[indices] = 0 
        prune_mask = prune_mask.bool()
        if self.max_radii2D.shape[0]!=self.get_xyz.shape[0]:
            self.max_radii2D = torch.zeros((self.get_xyz.shape[0])).to(self.get_xyz.device)
            
        n_gaussian = self.get_xyz.shape[0]
        self.prune_points(prune_mask)
        
        print('sample gaussians from', n_gaussian, 'to', self.get_xyz.shape[0])
...
class StandardGaussianModel(GaussianModel):
    ...
    def get_scaling(self):
        if self.use_isotropic_gs:
            scaling_repeat = self._scaling[...,:1].repeat(1,3)
        else:
            scaling_repeat = self._scaling
        scaling = scaling_repeat.mean()[None, None].expand_as(scaling_repeat) if self.all_the_same else scaling_repeat.mean(dim=1, keepdim=True).expand_as(scaling_repeat)
        return self.scaling_activation(scaling)

        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if scene.gaussians.use_isotropic_gs:
...
def skeleton_training_report(tb_writer, iteration, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc, renderArgs, deform, load2gpu_on_the_fly,  progress_bar=None):
    if tb_writer:
        tb_writer.add_scalar('iter_time', elapsed, iteration)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    d_values = deform.step(xyz.detach(), time_input, motion_mask=scene.gaussians.motion_mask)
